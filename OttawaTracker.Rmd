...
POP <- 996651 
Measurement <- as.integer(nrow(TargetPHU))

prevalencerate <- as.integer((Measurement / POP) * 100000)
valueBox(format(prevalencerate, big.mark = ","), icon = "ion-stats-bars")
```


### Overall Close Contact Cases
...
### Death Rate per 1M Population 
```{r}
fatality <- sum(outcomes$Fatal/POP)
fatality_rate_per_capita_K <- as.integer(fatality*1000000)
valueBox(fatality_rate_per_capita_K, icon = "ion-stats-bars")
```

### Recovery Rate per 1M Population 
```{r}
recovered <- sum(outcomes$Resolved/POP)
r_rate_per_capita_K <- as.integer(recovered*1000000)
...
### Unknown Cases
```{r}
valueBox(paste(100-(TRC+ORC+CCC), "%"), icon = "ion-help")
```
...
TargetPHU %>% 
  dplyr::filter(!is.na(Accurate_Episode_Date)) %>% 
  dplyr::filter(!Accurate_Episode_Date == "2019-12-25") %>%
  dplyr::filter(!Age_Group == "") %>% 
  dplyr::group_by(Accurate_Episode_Date, Age_Group) %>% 
  dplyr::count(name = "n") %>% 
  dplyr::ungroup() %>% 
  data.frame() %>% 
  dplyr::mutate(Age_Group = ordered(Age_Group, levels = c("<20", "20s", "30s", "40s", "50s", "60s", "70s", "80s", "90s"))) %>% 
  tidyr::spread(Age_Group, n, fill = 0) %>% 
  mutate(Total=Reduce("+",.[2:10]), 
         lessTwenty_Pct = round((`<20`/Total)*100,1), 
         twenty_Pct = round((`20s`/Total)*100,1), 
         thirty_Pct = round((`30s`/Total)*100,1), 
         forty_Pct = round((`40s`/Total)*100,1),
         fifty_Pct = round((`50s`/Total)*100,1),
         sixty_Pct = round((`60s`/Total)*100,1),
         seventy_Pct = round((`70s`/Total)*100,1),
         eighty_Pct = round((`80s`/Total)*100,1),
         ninety_Pct = round((`90s`/Total)*100,1)) %>% 
  ...
  plot_ly(x = ~Accurate_Episode_Date, 
          ...
          name = "90s",
          type = 'bar', 
          text = ~paste("90s", "<br>", ninety_Pct, "%",  " (n = ", `90s`, ")",sep = ""), 
          hoverinfo = "text", 
  ...
  add_trace(y = ~eighty_Pct, 
            name = "80s",
            type = 'bar', 
            text = ~paste("80s", "<br>", eighty_Pct, "%",  " (n = ", `80s`, ")",sep = ""), 
            hoverinfo = "text", 
  ...
  add_trace(y = ~seventy_Pct, 
            name = "70s",
            type = 'bar', 
            text = ~paste("70s", "<br>", seventy_Pct, "%",  " (n = ", `70s`, ")",sep = ""), 
            hoverinfo = "text", 
  ...
  add_trace(y = ~sixty_Pct, 
            name = "60s",
            type = 'bar', 
            text = ~paste("70s", "<br>", sixty_Pct, "%",  " (n = ", `60s`, ")",sep = ""), 
            hoverinfo = "text", 
  ...
  add_trace(y = ~fifty_Pct, 
            name = "50s",
            type = 'bar', 
            text = ~paste("70s", "<br>", fifty_Pct, "%",  " (n = ", `50s`, ")",sep = ""), 
            hoverinfo = "text", 
  ...
  add_trace(y = ~forty_Pct, 
            name = "40s",
            type = 'bar', 
            text = ~paste("40s", "<br>", forty_Pct, "%",  " (n = ", `40s`, ")",sep = ""), 
            hoverinfo = "text", 
  ...
  add_trace(y = ~thirty_Pct, 
            name = "30s",
            type = 'bar', 
            text = ~paste("30s", "<br>", thirty_Pct, "%",  " (n = ", `30s`, ")",sep = ""), 
            hoverinfo = "text", 
  ...
  add_trace(y = ~twenty_Pct, 
            name = "20s",
            type = 'bar', 
            text = ~paste("20s", "<br>", twenty_Pct, "%",  " (n = ", `20s`, ")",sep = ""), 
            hoverinfo = "text", 
  ...
  add_trace(y = ~lessTwenty_Pct, 
            name = "<20",
            type = 'bar', 
            text = ~paste("<20s", "<br>", lessTwenty_Pct, "%",  " (n = ", `<20`, ")",sep = ""), 
            hoverinfo = "text", 
...
### <b> Overall by Age Group (Linear) </b>
```{r csumbyAge}
TargetPHU %>% 
  dplyr::filter(!is.na(Accurate_Episode_Date)) %>% 
  dplyr::filter(!Accurate_Episode_Date == "2019-12-25") %>% 
...
TargetPHU %>% 
  ...
  dplyr::count(name = "n") %>% 
  dplyr::arrange(Accurate_Episode_Date, Client_Gender) %>% 
  dplyr::mutate(Client_Gender = trimws(Client_Gender)) %>% 
  dplyr::ungroup() %>% 
  data.frame() %>% 
  dplyr::group_by(Client_Gender) %>% 
