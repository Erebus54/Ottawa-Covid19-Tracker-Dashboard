...
outcomes <- TargetPHU %>% 
  ...
  dplyr::group_by(Case_Reported_Date, Outcome1) %>% 
...
plot_ly(data = outcomes_changes, x = ~Case_Reported_Date, y = ~new_resolved, 
               type = 'scatter',
               mode = "lines", 
               name = "New Recoveries", 
               line = list(color = "#25D366", width = 2)) %>% 
  
  add_trace(y = ~new_deaths, 
            name = "New Deaths", 
            line = list(color = "#FF5959", width = 2)) %>% 
  layout(title = "", 
...
acqBreakdown$colors <- c("#E01E5A", "#767676", "#F4B400", '#25D366', "#F4B400")
```
...
acq_daily$Case_AcquisitionInfo <- factor(acq_daily$Case_AcquisitionInfo, 
                                      levels = unique(acq_daily$Case_AcquisitionInfo)[order(acq_daily$count, decreasing = T)])
...
#breakdown by gender 
genders <- TargetPHU %>% 
  dplyr::filter(Client_Gender == "MALE" | Client_Gender == "FEMALE") %>% 
...
#### <b> Code </b>
Codebase and dataset used to generate this web application are available on [Github](https://github.com/Erebus54/Ottawa-Covid19-Tracker-Dashboard).
